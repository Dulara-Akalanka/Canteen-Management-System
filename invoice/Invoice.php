...
class Invoice{
	...
	public function saveInvoice($POST) {	
		...
		for ($i = 0; $i < count($POST['productName']); $i++) {
			$sqlInsertItem = "
			INSERT INTO ".$this->invoiceOrderItemTable."( item_name, order_item_quantity, order_item_price, order_item_final_amount) 
			VALUES ('".$POST['productName'][$i]."', '".$POST['quantity'][$i]."', '".$POST['price'][$i]."', '".$POST['total'][$i]."')";			
	...
	// public function saveInvoice($POST) {		
	// 	$sqlInsert = "
	// 		INSERT INTO ".$this->invoiceOrderTable."(user_id, order_receiver_name, order_total_before_tax, order_total_tax, order_tax_per, order_total_after_tax) 
	// 		VALUES ('".$POST['userId']."', '".$POST['companyName']."',  '".$POST['subTotal']."', '".$POST['taxAmount']."', '".$POST['taxRate']."', '".$POST['totalAftertax']."')";		
	// 	mysqli_query($this->dbConnect, $sqlInsert);
	// 	$lastInsertId = mysqli_insert_id($this->dbConnect);
